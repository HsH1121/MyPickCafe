...
- **RAG + LLM 기반 자연어 카페 추천과 검색 다양성 제약** — 사용자의 자연어 요청을 `bge-m3`로 임베딩해 ChromaDB에서 유사 리뷰를 검색하고, 그 결과를 컨텍스트로 `qwen2.5:14b`가 추천 카페를 생성합니다. 이때 한 카페가 검색 결과를 독점하지 않도록 카페별 리뷰 수에 상한을 뒀습니다(1위 카페 최대 5건, 순위가 내려갈수록 1건씩 감소). → [상세](ARCHITECTURE.md#주요-동작-흐름)
