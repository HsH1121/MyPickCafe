...
for i in range(1, MEMBER_COUNT + 1):
    ...
    sql_lines.append(
        ...
        f"VALUES ('user{i:03d}@test.com', '{TEST_PASSWORD_HASH}', 'user{i:03d}', {age}, '{gender}', 'MEMBER', SYSTIMESTAMP, 0);"
    )
    print(f"  [{i:02d}/{MEMBER_COUNT}] user{i:03d}")

with open("member_dummy.sql", "w", encoding="utf-8") as f:
    f.write("\n".join(sql_lines))

print(f"\n🎉 member_dummy.sql 저장 완료! (총 {MEMBER_COUNT}명)")
