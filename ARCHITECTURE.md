...
4. **리뷰 검색**: 조건 문장을 `bge-m3`로 임베딩해 후보 카페의 리뷰 중 코사인 유사도로 15건을 찾습니다. 인덱스에는 리뷰 본문만 임베딩합니다(카페명·주소 제외). 한 카페가 결과를 독점하지 않도록 카페별 리뷰 수에 상한을 둡니다(1위 카페 최대 5건, 순위가 내려갈수록 1건씩 감소).
5. **LLM 2차 호출**: 카페별 최고 점수 기준 상위 5곳의 리뷰만(이름·주소 없이) 조건 문장과 함께 넘깁니다. LLM은 조건을 요구사항으로 나누고 카페마다 충족·누락을 판정합니다. 코드가 **요구사항을 모두 충족한 카페를 전부**(유사도 순) 반환하고, 모두 충족한 카페가 없으면 **유사도 1위 1곳만** 반환합니다. 추천 이유는 충족한 조건만 근거로 씁니다. 카페명·주소는 검색 결과에서 채우고, 후보에 없는 cafeId는 버립니다. 호출이 실패하면 검색 결과를 그대로 반환합니다.
...
사용자가 마이페이지에서 고른 니즈 태그 집합과 각 카페의 태그 집합 사이의 **자카드 유사도**를 계산해 높은 순으로 보여줍니다 (`RecommendService`). 메인 페이지와 `/cafes?sort=recommend`에서 사용합니다. 후보는 승인된(`APPROVED`) 카페로 한정하며, 겹치는 태그가 하나도 없는 카페는 제외합니다.
